...
static int L_WIFI_MODE(lua_State *L) {
    int mode = luaL_optinteger(L, 1, WIFI_MODE_STA); // default = STA
    ...
    switch (mode) {
        ...
        case WIFI_MODE_APSTA:
            ...
            break;
        default:
            return luaL_error(L, "invalid wifi mode");
    }
...
int LUA_OPEN_WIFI(lua_State *L) {
    ...
    lua_pushinteger(L, WIFI_MODE_STA);   lua_setfield(L, -2, "STA");
    lua_pushinteger(L, WIFI_MODE_AP);    lua_setfield(L, -2, "AP");
    lua_pushinteger(L, WIFI_MODE_APSTA); lua_setfield(L, -2, "STA_AP");
